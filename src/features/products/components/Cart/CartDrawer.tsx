      ...
      {cartOpen && (
        <>
          ...
          <motion.div
            ...
            className={`fixed ${isRTL ? "left-0" : "right-0"} top-0 bottom-0 w-full max-w-md bg-cream z-[70] flex flex-col shadow-2xl`}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-5 border-b border-espresso/10 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-black text-espresso uppercase tracking-widest">
              ...
              <button
                ref={closeButtonRef}
                onClick={handleClose}
                className="w-11 h-11 flex items-center justify-center rounded-xl hover:bg-espresso/8 transition-colors"
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {cartProducts.length === 0 ? (
                  ...
                  >
                    ...
                    <div className="relative z-10 flex flex-col items-center">
                      ...
                      <p className="text-espresso/65 text-xs mt-2 mb-5">
                        {t("cart.emptyHint")}
                      </p>
                      <Link
                  ...
                  <AnimatePresence initial={false}>
                    {cartProducts.map(({ item, product }) => (
                      ...
                      >
                        {/* Image */}
                        <div className="w-20 h-20 rounded-xl bg-linen overflow-hidden shrink-0">
                          {product.images[0] && (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm font-black text-espresso tracking-tight mt-0.5 truncate">
                            {product.name}
                          </p>
                          ...
                          <div className="flex items-center justify-between mt-3">
                            <div className="flex items-center gap-2 bg-cream rounded-lg p-1">
                              ...
                              >
                                <Minus size={11} className="text-espresso" />
                              </button>
                              <span className="text-xs font-black text-espresso w-4 text-center">
                                {item.quantity}
                              </span>
                              <button
                                onClick={() => updateQuantity(item.productId, item.quantity + 1)}
                              ...
                              >
                                <Plus size={11} className="text-espresso" />
                              </button>
                            </div>
                            <button
              ...
              {/* Order section */}
              {cartProducts.length > 0 && (
                <div className="px-6 pb-8 border-t border-espresso/10 pt-5 space-y-4">

                  {/* Totals */}
                  <div className="space-y-2.5">
                    <div className="flex justify-between text-xs text-espresso/65">
                      <span>{t("cart.subtotal")}</span>
                      <span>{total.toLocaleString()} MAD</span>
                    </div>
                    <div className="flex items-start justify-between gap-2 bg-gold/6 border border-gold/15 rounded-xl px-3 py-2.5">
                      <div>
                        <p className="text-xs font-black text-espresso/70">{t("cart.deliveryEstimate")}</p>
                        <p className="text-[9px] text-espresso font-black uppercase tracking-wider mt-0.5">
                          {isFreeDelivery
                  ...
                  <button
                    ...
                    className="w-full bg-espresso text-white py-4 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-espresso-light active:scale-95 transition-all"
                  >
                  ...
                  {showOrderForm && (
                    ...
                    >
                      <p className="text-[10px] font-black uppercase tracking-widest text-espresso/65 mb-5">
                  ...
                  {!showOrderForm && (
                    <button
                      ...
                      className="w-full border border-whatsapp text-espresso py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-whatsapp/8 active:scale-95 transition-all"
                    >
                  ...
                  {!showOrderForm && (
                    <button
                      onClick={clearCart}
                      className="w-full text-espresso/65 text-[9px] font-black uppercase tracking-widest hover:text-espresso/65 transition-colors"
                    >
                      {t("cart.clearCart")}
                    </button>
                  )}
